    ...
    <section style={{ background: 'var(--color-bg-dark)' }}>
      <div className="fp-container" style={{ padding: 'var(--section-py) 0' }}>
        <div className="section-inner">
          <p style={{ fontFamily: 'var(--font-sans)', fontSize: 11, fontWeight: 500, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.4)' }}>
            A SOLUÇÃO
          </p>
          <h2 style={{ fontFamily: 'var(--font-sans)', fontWeight: 500, fontSize: 'clamp(32px, 4vw, 52px)', letterSpacing: '-0.025em', lineHeight: 1.1, color: '#FFFFFF', maxWidth: 560, marginTop: 24, marginBottom: 64 }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3" style={{ gap: 1 }}>
            {items.map((item) => (
              <div key={item.num} style={{ background: 'rgba(255,255,255,0.04)', padding: '40px 32px', borderTop: '1px solid rgba(255,255,255,0.08)' }}>
                ...
                <p style={{ fontFamily: 'var(--font-sans)', fontWeight: 500, fontSize: 20, color: '#FFFFFF', letterSpacing: '-0.01em', marginTop: 24, marginBottom: 12 }}>{item.title}</p>
                <p style={{ fontFamily: 'var(--font-sans)', fontSize: 14, fontWeight: 400, color: 'rgba(255,255,255,0.5)', lineHeight: 1.7 }}>{item.desc}</p>
              </div>
            ))}
          </div>
        </div>

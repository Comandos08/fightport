  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.4)', padding: 16 }} onClick={onClose}>
      <div
        ...
        style={{ background: '#FFFFFF', borderRadius: 'var(--radius-md)', padding: 40, maxWidth: 320, width: '100%', textAlign: 'center' }}
      >
        ...
        <p style={{ fontFamily: 'var(--font-sans)', fontSize: 13, color: 'var(--color-text-muted)', marginTop: 16 }}>{t('passport.qrScan')}</p>
        <button
          onClick={onClose}
          className="cursor-pointer"
          style={{ background: 'transparent', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-sm)', padding: '8px 20px', fontFamily: 'var(--font-sans)', fontWeight: 400, fontSize: 14, color: 'var(--color-text-muted)', transition: 'var(--transition)', marginTop: 20 }}
...
export default function PassportPage() {
    ...
    return (
      <div style={{ minHeight: '100vh', background: 'var(--color-bg)' }}>
        ...
        <div style={{ padding: '80px 24px', textAlign: 'center', maxWidth: 400, margin: '0 auto' }}>
          <h1 style={{ fontFamily: 'var(--font-sans)', fontWeight: 500, fontSize: 28, color: 'var(--color-text)', marginBottom: 8 }}>{t('passport.notFoundTitle')}</h1>
  ...
  const actionBtnStyle: React.CSSProperties = {
    ...
    padding: '10px 20px', fontFamily: 'var(--font-sans)', fontWeight: 400, fontSize: 14,
    color: 'var(--color-text-muted)', transition: 'var(--transition)', display: 'inline-flex', alignItems: 'center', gap: 8,
  };
    ...
    <div style={{ minHeight: '100vh', background: 'var(--color-bg)' }}>
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '64px 24px 80px' }}>
        ...
        <div style={{ background: 'var(--color-bg-soft)', borderRadius: 'var(--radius-sm)', padding: '14px 20px', textAlign: 'center', marginTop: 32 }}>
          <span style={{ fontFamily: 'var(--font-sans)', fontSize: 13, color: 'var(--color-text-muted)' }}>{t('passport.idLabel')}: </span>
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap', marginTop: 32 }}>
          <button onClick={share} className="cursor-pointer" style={actionBtnStyle}
            ...
            <Share2 style={{ width: 14, height: 14 }} /> {t('passport.share')}
          </button>
          <button onClick={() => setShowQr(true)} className="cursor-pointer" style={actionBtnStyle}
            onMouseEnter={e => { e.currentTarget.style.background = 'var(--color-bg-soft)'; e.currentTarget.style.borderColor = 'var(--color-border-dark)'; }}
            onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.borderColor = 'var(--color-border)'; }}>
            <QrCode style={{ width: 14, height: 14 }} /> {t('passport.qrCode')}
          </button>
        </div>
        ...
        <div style={{ paddingTop: 48 }}>
          ...
          <div>
            ...
            {achievements.length === 0 && (
              <p style={{ fontFamily: 'var(--font-sans)', fontSize: 13, color: 'var(--color-text-muted)' }}>{t('passport.noAchievements')}</p>
            )}
          </div>
        </div>

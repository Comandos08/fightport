...
export default function NovaConquistaPage() {
      ...
      <div style={{ padding: '32px 32px', maxWidth: 600 }}>
        <div style={{ textAlign: 'center', padding: '48px 0' }}>
          <div style={{ width: 64, height: 64, margin: '0 auto 16px', borderRadius: '50%', background: 'var(--color-bg-soft)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <CheckCircle style={{ width: 32, height: 32, color: '#2D6A4F' }} />
          </div>
          <h1 style={{ fontFamily: 'var(--font-sans)', fontWeight: 500, fontSize: 24, color: 'var(--color-text)', letterSpacing: '-0.02em', marginBottom: 8 }}>Conquista registrada!</h1>
          <p style={{ fontFamily: 'var(--font-sans)', fontSize: 14, color: 'var(--color-text-muted)', marginBottom: 32 }}>A graduação foi registrada com sucesso.</p>
          ...
          <div style={{ background: 'var(--color-bg-soft)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-sm)', padding: 24, marginBottom: 24, textAlign: 'left', maxWidth: 440, margin: '0 auto 24px' }}>
            <label style={labelStyle}>Hash de verificação</label>
            <div className="flex items-center" style={{ gap: 8, marginBottom: 16 }}>
              ...
              <button onClick={() => { navigator.clipboard.writeText(generatedHash); toast.success('Hash copiada'); }} className="shrink-0 cursor-pointer" style={{ background: 'none', border: 'none' }} aria-label="Copiar hash">
                <Copy style={{ width: 16, height: 16, color: 'var(--color-text-muted)', transition: 'var(--transition)' }} />
              </button>
            </div>
          </div>
  ...
  return (
    <div style={{ padding: '32px 32px', maxWidth: 1100 }}>
      <h1 style={{ fontFamily: 'var(--font-sans)', fontWeight: 400, fontSize: 28, color: 'var(--color-text)', letterSpacing: '-0.02em', marginBottom: 32 }}>Registrar Conquista</h1>
      ...
      >
        Você tem {balance} créditos · Cada graduação consome 1 crédito
      </div>

      {balance === 0 && (
        <div style={{ border: '1px solid var(--color-border)', borderRadius: 'var(--radius-sm)', padding: 24, textAlign: 'center', marginBottom: 24, background: 'var(--color-bg-soft)' }}>
          <p style={{ fontFamily: 'var(--font-sans)', fontSize: 14, color: 'var(--color-text)', marginBottom: 8 }}>Sem créditos disponíveis.</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5" style={{ gap: 32 }}>
        <div className="lg:col-span-3" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div className="relative">
            <label style={labelStyle}>Praticante</label>
            {selectedPractitioner ? (
              <div className="flex items-center" style={{ ...inputStyle, height: 44, display: 'flex', gap: 12, background: 'var(--color-bg-soft)' }}>
                <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'var(--color-text)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-sans)', fontWeight: 500, fontSize: 10 }}>
              ...
              <>
                ...
                {searchResults.length > 0 && (
                  <div className="absolute z-10 mt-1 w-full" style={{ background: 'var(--color-bg)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-sm)', boxShadow: '0 10px 40px -10px rgba(0,0,0,0.15)' }}>
                    {searchResults.map(a => (
                      ...
                      >
                        ...
                        <span style={{ fontFamily: 'var(--font-sans)', fontSize: 14, color: 'var(--color-text)' }}>{a.first_name} {a.last_name}</span>
                      </button>
                    ))}
                  </div>
                )}
          ...
          <div>
            <label style={labelStyle}>Data da graduação</label>
          ...
          <div>
            <label style={labelStyle}>Quem graduou</label>
          ...
          <div>
            <label style={labelStyle}>Texto complementar (opcional)</label>
            <textarea value={note} onChange={e => setNote(e.target.value)} rows={3} style={{ ...inputStyle, resize: 'none', padding: '12px 16px' }} placeholder="Ex: Promovido a Faixa Azul..." onFocus={focusInput as any} onBlur={blurInput as any} />
          </div>
          ...
          <button
            ...
            style={{
              background: canSubmit ? 'var(--color-bg-amber)' : 'var(--color-border)',
              color: canSubmit ? '#1C1C1C' : 'var(--color-text-muted)',
              fontFamily: 'var(--font-sans)',
              fontWeight: 500,
              fontSize: 15,
              padding: '14px 28px',
        ...
        <div className="lg:col-span-2">
          <p style={{ fontFamily: 'var(--font-sans)', fontWeight: 500, fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--color-text-muted)', marginBottom: 12 }}>Preview</p>
            ...
            {selectedPractitioner && belt ? (
              <>
                <div className="flex items-center" style={{ gap: 8, marginBottom: 12 }}>
                  ...
                  <span style={{ fontSize: 11, fontFamily: 'var(--font-sans)', fontWeight: 500, color: '#2D6A4F' }}>Verificado</span>
                </div>
                <p style={{ fontFamily: 'var(--font-sans)', fontWeight: 500, fontSize: 14, color: 'var(--color-text)', margin: 0 }}>{selectedPractitioner.first_name} {selectedPractitioner.last_name}</p>
                <p style={{ fontFamily: 'var(--font-sans)', fontSize: 12, color: 'var(--color-text-muted)', margin: '2px 0 12px' }}>{school?.name}</p>
                <div className="flex items-center" style={{ gap: 8, marginBottom: 8 }}>
                  <BeltBadge belt={belt as any} size="sm" />
                  <span style={{ fontFamily: 'var(--font-sans)', fontSize: 12, color: 'var(--color-text-muted)' }}>{date ? formatDate(date) : ''}</span>
                </div>
                <p style={{ fontFamily: 'var(--font-sans)', fontSize: 12, color: 'var(--color-text-muted)', margin: 0 }}>{graduatedBy}</p>
                {note && <p style={{ fontFamily: 'var(--font-sans)', fontSize: 12, color: 'var(--color-text-muted)', fontStyle: 'italic', marginTop: 8 }}>{note}</p>}
              </>
            ) : (
              <p style={{ fontFamily: 'var(--font-sans)', fontSize: 13, color: 'var(--color-text-muted)', textAlign: 'center', padding: '32px 0' }}>Selecione um atleta e faixa para ver o preview</p>
            )}
          </div>
        </div>
      </div>
      ...
      {showConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(15,25,35,0.5)', padding: 16 }} onClick={() => setShowConfirm(false)}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'var(--color-bg)', borderRadius: 'var(--radius-sm)', padding: 24, maxWidth: 400, width: '100%', boxShadow: '0 20px 60px -15px rgba(0,0,0,0.2)' }}>
            <h3 style={{ fontFamily: 'var(--font-sans)', fontWeight: 500, fontSize: 18, color: 'var(--color-text)', marginBottom: 8 }}>Confirmar registro</h3>

...
export default function NovaConquistaPage() {
      ...
      <div style={{ padding: '32px 32px', maxWidth: 600 }}>
        <div style={{ textAlign: 'center', padding: '48px 0' }}>
          ...
          <div className="flex flex-col sm:flex-row justify-center" style={{ gap: 12 }}>
            ...
            <Button onClick={() => { setShowSuccess(false); setSelectedPractitioner(null); setBelt(''); setSearchText(''); setNote(''); setMobileStep(1); }}>{t('achievement.success.registerAnother')}</Button>
          </div>
        </div>
      </div>
    );
  ...
  const canAdvanceStep1 = !!selectedPractitioner;
  const canAdvanceStep2 = !!belt && !!date && !!graduatedBy.trim();
  ...
  const stepLabels = [
    t('achievement.practitioner'),
    `${t('achievement.belt')} / ${t('achievement.date')}`,
    t('achievement.confirm.title'),
  ];

  return (
    <div className="px-4 py-6 sm:px-8 sm:py-8" style={{ maxWidth: 1100 }}>
      <h1 className="text-2xl sm:text-[28px]" style={{ fontFamily: 'var(--font-sans)', fontWeight: 400, color: 'var(--color-text)', letterSpacing: '-0.02em', marginBottom: 24 }}>{t('achievement.title')}</h1>
      <div style={{ background: 'var(--color-bg-soft)', borderBottom: '1px solid var(--color-border)', padding: '14px 16px', marginBottom: 20, fontFamily: 'var(--font-sans)', fontWeight: 400, fontSize: 14, color: 'var(--color-text-muted)' }}>{t('achievement.creditInfo', { balance })}</div>
      ...
      <div className="sm:hidden">
        ...
        <div className="flex items-center justify-between" style={{ marginBottom: 24 }}>
          {[1, 2, 3].map((n, i) => {
            ...
            return (
              <div key={n} className="flex items-center" style={{ flex: i < 2 ? 1 : '0 0 auto', gap: 8 }}>
                <div style={{
                  width: 28, height: 28, borderRadius: '50%',
                  background: isActive ? 'var(--color-text)' : isDone ? '#2D6A4F' : 'var(--color-bg-soft)',
                  color: isActive || isDone ? '#fff' : 'var(--color-text-muted)',
                  border: isActive || isDone ? 'none' : '1px solid var(--color-border)',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontFamily: 'var(--font-sans)', fontWeight: 600, fontSize: 12, flexShrink: 0,
                }}>
                  {isDone ? <Check style={{ width: 14, height: 14 }} /> : n}
                </div>
                {i < 2 && <div style={{ flex: 1, height: 1, background: isDone ? '#2D6A4F' : 'var(--color-border)' }} />}
              </div>
            );
          })}
        </div>
        <p style={{ fontFamily: 'var(--font-sans)', fontSize: 12, fontWeight: 500, color: 'var(--color-text-muted)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 16 }}>
          {`${mobileStep}/3 · ${stepLabels[mobileStep - 1]}`}
        </p>

        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          {mobileStep === 1 && PractitionerField}
          {mobileStep === 2 && (
            <>
              {BeltField}
              {DateField}
          ...
          {mobileStep === 3 && (
            <>
              <p style={{ fontFamily: 'var(--font-sans)', fontWeight: 500, fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.1em', color: 'var(--color-text-muted)' }}>{t('achievement.preview.label')}</p>
              {PreviewCard}
            </>
          )}
        </div>
        ...
        <div className="flex items-center justify-between" style={{ marginTop: 24, gap: 12 }}>
          <Button
            variant="ghost"
            onClick={() => setMobileStep(s => (s > 1 ? ((s - 1) as 1 | 2 | 3) : s))}
            disabled={mobileStep === 1}
          >
            <ChevronLeft className="h-4 w-4 mr-1" /> {t('achievement.confirm.cancel')}
          </Button>
          {mobileStep < 3 ? (
            <button
              onClick={() => setMobileStep(s => ((s + 1) as 1 | 2 | 3))}
              disabled={(mobileStep === 1 && !canAdvanceStep1) || (mobileStep === 2 && !canAdvanceStep2)}
              style={{
                background: ((mobileStep === 1 && canAdvanceStep1) || (mobileStep === 2 && canAdvanceStep2)) ? 'var(--color-bg-amber)' : 'var(--color-border)',
                color: ((mobileStep === 1 && canAdvanceStep1) || (mobileStep === 2 && canAdvanceStep2)) ? '#1C1C1C' : 'var(--color-text-muted)',
                fontFamily: 'var(--font-sans)', fontWeight: 500, fontSize: 14,
                padding: '12px 20px', borderRadius: 'var(--radius-sm)', border: 'none',
                cursor: ((mobileStep === 1 && canAdvanceStep1) || (mobileStep === 2 && canAdvanceStep2)) ? 'pointer' : 'not-allowed',
      ...
      {/* ===== Desktop layout (>= sm) ===== */}
      <div className="hidden sm:grid grid-cols-1 lg:grid-cols-5" style={{ gap: 32 }}>
